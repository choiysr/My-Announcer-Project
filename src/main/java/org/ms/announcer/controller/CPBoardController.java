...
public class CPBoardController {
    ...
    public void register(@RequestBody CPBoard[] board) {
    ...
    public ResponseEntity<List<String>> registerFiles(MultipartFile[] files) {
        ...
        try{
            for(MultipartFile file : files) {
                String fileNameWithoutType = file.getOriginalFilename().substring(0,file.getOriginalFilename().lastIndexOf("."));
                list.add(audioSave(fileNameWithoutType, file.getBytes()).replace("\\", "*"));
            }
        } catch(Exception e) {
            e.printStackTrace();
        }
